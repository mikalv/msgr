...
2. **Flutter client readiness**
   - Prune unused routes from `AppNavigation` and ensure first-run onboarding always lands testers in the chat flow.
   - Add recovery UI for expired/failed OTP verification and allow bootstrap retries when channel fetches or realtime handshakes fail.
   - Cover offline upload/error scenarios with widget or integration tests so optimistic updates roll back cleanly.
3. **Developer experience**
   - Document required Erlang/Elixir/Flutter toolchain versions (e.g., via `.tool-versions` or Docker) and add setup scripts for the alpha stack.
   - Introduce an end-to-end Flutter integration test that provisions accounts, sends a message, and verifies timeline rendering against a live backend.
   - Provide smoke tests or CI checks that confirm bridge daemons and StoneMQ queues are reachable before merging changes.
