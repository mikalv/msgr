defmodule MessngrWeb.FamilyShoppingListController do
  ...
  def index(conn, %{"family_id" => family_id} = params) do
    ...
    with _ <- FamilySpace.ensure_membership(family_id, current_profile.id) do
      lists = FamilySpace.list_shopping_lists(family_id, include_archived: include_archived?)
  ...
  def create(conn, %{"family_id" => family_id, "list" => list_params}) do
    ...
    attrs =
      list_params
      |> Map.new()
      |> Map.put_new(:name, list_params["name"])

    with {:ok, list} <- FamilySpace.create_shopping_list(family_id, current_profile.id, attrs) do
      conn
      |> put_status(:created)
  ...
  def show(conn, %{"family_id" => family_id, "id" => list_id}) do
    ...
    with _ <- FamilySpace.ensure_membership(family_id, current_profile.id),
         list <- FamilySpace.get_shopping_list!(family_id, list_id) do
  ...
  def delete(conn, %{"family_id" => family_id, "id" => list_id}) do
    ...
    with {:ok, _} <- FamilySpace.delete_shopping_list(family_id, list_id, current_profile.id) do
