defmodule MessngrWeb.RTCChannelTest do
  ...
  test "participants receive offers", %{
    conversation: conversation,
    host_profile: host_profile,
    host_account: host_account,
    peer_profile: peer_profile,
    ...
    {:ok, _peer_response, _peer_socket} =
      subscribe_and_join(peer_socket, RTCChannel, "rtc:#{conversation.id}", %{
    ...
    assert_broadcast "signal:offer", %{
      "sdp" => "offer",
      "from" => host_profile.id,
      "target" => ^peer_profile.id
    }
  end

  test "leaving ends call when host disconnects", %{
    conversation: conversation,
    host_profile: host_profile,
    host_account: host_account,
    peer_profile: peer_profile,
  ...
  test "direct calls refuse a third participant", %{
    conversation: conversation,
    host_profile: host_profile,
    host_account: host_account,
    peer_profile: peer_profile,
    ...
    assert {:error, %{reason: "direct_call_full"}} =
             subscribe_and_join(observer_socket, RTCChannel, "rtc:#{conversation.id}", %{
               "call_id" => host_response["call_id"]
             })
  end
end
