defmodule MainProxy do
  ...
  def should_start_server?() do
    ...
    # - enable `server: true` option for edge_router (by default)
    # - run `iex -S mix phx.server`
    # - run `mix phx.server`
    Application.get_env(:phoenix, :serve_endpoints, false) ||
      Application.get_env(:edge_router, :server, true)
  end
  ...
  def spec(handler_opts, callback_module) do
    Enum.reduce([:http, :https], [], fn scheme, result ->
      case Application.get_env(:edge_router, scheme) do
        nil ->
        ...
        scheme_opts ->
          ...
          Logger.info("[edge_router] Listening on #{scheme} with options: #{inspect(opts)}")
  ...
  def default_fetch_backends do
    case Application.fetch_env(:edge_router, :backends) do
      {:ok, backends} ->
        backends

      :error ->
        Logger.warning(
          "No backends specified. Either configure :edge_router, :backends or define a " <>
  ...
  defp port_to_integer(:undefined),
    do: raise("port is missing from the edge_router configuration")
