defmodule Teams.ClientBootstrap do
  ...
  @spec build_bootstrap_payload_for(String.t(), Teams.TenantModels.Profile.t()) :: %{channels: [map], conversations: [map], messages: [map], profiles: [map], team: map}
  def build_bootstrap_payload_for(tenant, %Profile{} = profile) do
    ...
    messages = List.flatten(Enum.map(channels, fn channel ->
      Teams.TenantModels.Message.get_for_channel(tenant, channel.id) |> Enum.map(&filter_msg_for_json(&1))
    end))

    %{channels: channels, conversations: conversations, messages: messages, profiles: profiles, team: tenant}
  end
