defmodule TeamsWeb.MiddleLayers.InviteLayer do
  ...
  """
  @spec invite_user(String.t(), Teams.TenantModels.Profile.t(), String.t()) ::
          {:ok, Teams.TenantModels.Invitation.t()}
  def invite_user(teamName, profile, whotoinvite) do
    {:ok, invitation} =
      case is_identifier_email?(whotoinvite) do
        true ->
          {:ok, res} =
            Teams.TenantModels.Invitation.create_email_invitation(teamName, profile, whotoinvite)
        ...
        false ->
          {:ok, res} =
            Teams.TenantModels.Invitation.create_msisdn_invitation(teamName, profile, whotoinvite)
