defmodule TeamsWeb.PublicApiController do
  ...
  def select_team(conn, params) do
    ...
    case Teams.TenantTeam.am_i_a_member?(ttstr, uid) do
      {true, team} ->
        ...
        {:ok, token, claims} =
          AuthProvider.Guardian.issue_token_for_team(ttstr, team.id, uid, profile_id)
      ...
      {false, nil} ->
        Logger.warning(
          "User uid=#{uid} attempted to authenticate to a team (#{ttstr}) which the user isn't a member of."
        )

        conn
        |> send_resp(
          401,
          Jason.encode!(%{"status" => "error", "error" => "You're not a member of this team!"})
        )
    end
  end
  ...
  def create_team(conn, params) do
    ...
    if Teams.TenantsHelper.is_tenant_name_available(name) do
      ...
      conn
      |> send_resp(200, Jason.encode!(%{"status" => "ok", "team" => filter_team_for_json(team)}))
    else
      conn
      |> send_resp(
        400,
        Jason.encode!(%{"status" => "error", "error" => "Team name is already taken!"})
      )
    end
  end
