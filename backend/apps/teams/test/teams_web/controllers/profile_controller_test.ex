defmodule TeamsWeb.Subdomain.ProfileControllerTest do
  ...
  describe "list/2" do
    test "lists all profiles", %{conn: conn, tenant: tenant} do
      ...
      assert json_response(conn, 200) ==
               Enum.map(profiles, &TeamsWeb.Subdomain.ProfileController.filter_profile_for_json/1)
    end
  end
  ...
  describe "create/2" do
    test "creates a new profile", %{conn: conn, tenant: tenant} do
      ...
      |> expect(:quick_create_profile, fn ^tenant, "user_id", "new_user", "First", "Last" ->
        %Profile{id: 1, username: "new_user"}
      end)
      |> expect(:load_roles, fn ^tenant, %Profile{id: 1} ->
        %Profile{id: 1, username: "new_user"}
      end)
      ...
      ProfileRole
      |> expect(:upsert_profile_roles, fn ^tenant, 1, [1] ->
        {:ok, %Profile{id: 1, username: "new_user"}}
      end)

      conn = post(conn, Routes.profile_path(conn, :create), params)

      assert json_response(conn, 200) ==
               TeamsWeb.Subdomain.ProfileController.filter_profile_for_json(%Profile{
                 id: 1,
                 username: "new_user"
               })
    end
  ...
  describe "update/2" do
    test "updates own profile", %{conn: conn, tenant: tenant} do
      ...
      |> expect(:update, fn ^tenant,
                            ^authed_profile,
                            %{first_name: "Updated", last_name: "Name", settings: nil} ->
        {:ok, %Profile{id: 1, username: "authed_user", first_name: "Updated", last_name: "Name"}}
      end)
    ...
    test "returns error if trying to update another profile without permission", %{
      conn: conn,
      tenant: tenant
    } do
      claims = %{"sub" => "user_id"}
      ...
      assert json_response(conn, 401) == %{
               "error" => "You're not allowed to update someone else's profile!"
             }
    end
  end
