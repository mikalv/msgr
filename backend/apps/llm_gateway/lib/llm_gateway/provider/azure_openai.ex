defmodule LlmGateway.Provider.AzureOpenAI do
  @moduledoc """
  ...
  def chat_completion(%Request{} = request, opts) do
    Telemetry.emit(:provider_call_started, %{provider: :azure_openai})
    ...
    with {:ok, body} <- encode_body(request),
      ...
      Telemetry.emit(:provider_call_finished, %{provider: :azure_openai})
      {:ok, parsed}
    end
  end
  ...
  defp chat_url(opts) do
    ...
    String.trim_trailing(endpoint, "/") <> "/openai/deployments/#{deployment}/chat/completions?api-version=#{api_version}"
  end
  ...
  defp encode_body(%Request{} = request) do
    body =
      ...
      |> put_if_present(:model, request.model)
