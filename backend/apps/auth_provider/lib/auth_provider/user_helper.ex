defmodule AuthProvider.UserHelper do
  ...
  @spec validate_login_code_for_user(integer(), %User{}) ::
          {:ok, :valid_code} | {:error, :invalid_code}
  def validate_login_code_for_user(code, %User{} = user) do
    auth_code =
      Repo.get_by(AuthMethod, user_id: user.id, auth_type: "one_time_code", is_disabled: false)
  ...
  def send_msisdn_auth_code("+" <> msisdn, message) do
    if System.get_env("BULK_SMS_ENABLE", "false") == "true" do
      ...
      unless is_nil(password) and is_nil(username) do
        query =
          "username=#{username}&password=#{password}&msisdn=#{msisdn}&sender=Msgr&message=#{message}"
  ...
  def generate_one_time_code(),
    do: :io_lib.format("~6..0B", [:rand.uniform(10_000_00) - 1]) |> List.to_string()
end
