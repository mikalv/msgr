defmodule AuthProvider.UserHelperTest do
  ...
  describe "find_or_register_user_by_email/2" do
    test "registers a new user if email not found" do
      with_mock Repo,
        ...
        insert: fn _ -> {:ok, %User{id: 1, email: "test@example.com"}} end do
        assert {:ok, %User{id: 1, email: "test@example.com"}} =
                 UserHelper.find_or_register_user_by_email("test@example.com")
      end
    end
  ...
  describe "find_or_register_user_by_msisdn/2" do
    test "registers a new user if msisdn not found" do
      with_mock Repo,
        get_by: fn _, _ -> nil end,
        insert: fn _ -> {:ok, %User{id: 1, msisdn: "1234567890"}} end do
        assert {:ok, %User{id: 1, msisdn: "1234567890"}} =
                 UserHelper.find_or_register_user_by_msisdn("1234567890")
      end
    end
  ...
  describe "create_login_code_for_user/1" do
    test "creates a new login code for user" do
      ...
      with_mock Repo,
        insert_or_update: fn _ ->
          {:ok, %AuthMethod{auth_type: "one_time_code", user_id: user.id, value: "123456"}}
        end do
  ...
  describe "validate_login_code_for_user/2" do
    test "validates correct login code" do
      ...
      with_mock Repo,
        get_by: fn _, _ -> %AuthMethod{value: "123456"} end,
        update: fn _ -> {:ok, %AuthMethod{}} end do
