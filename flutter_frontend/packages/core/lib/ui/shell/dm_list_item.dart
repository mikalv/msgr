...
class DmListItem extends StatelessWidget {
  ...
  void _showContextMenu(BuildContext context, Offset globalPosition) async {
    final items = <PopupMenuEntry<String>>[
      _menuItem('star', isStarred ? Icons.star : Icons.star_outline,
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
          content: Text('Coming soon'), duration: Duration(seconds: 2)),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final textColor = isSelected
        ...
        : hasUnread
            ...
            : t.sidebarText;
    final fontWeight =
        (hasUnread || isSelected) ? FontWeight.w600 : FontWeight.w400;
    ...
    return GestureDetector(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          child: Row(
            children: [
              // Avatar: group icon for 3+ members, regular avatar for 1:1
              if (contact.memberCount > 2)
                Container(
                  width: MsgrDimensions.sidebarAvatarSize,
                  height: MsgrDimensions.sidebarAvatarSize,
                  decoration: BoxDecoration(
                    color: t.accent.withAlpha(40),
                    borderRadius: BorderRadius.circular(6),
                  ),
                  child: Icon(Icons.group, size: 14, color: t.accent),
                )
              else
                ProfileAvatar(
